...
module.exports = app => {
  ...
  app.post("/api/team/join-team", checkUserInTeam, (req, res) => {
    ...
    Team.findOneAndUpdate(
      { _id: id },
      { $push: { membersId: req.body.userEmail } },
      { new: true }
    )
      .then(docs => {
        res.send(docs);
  ...
  app.post("/api/team/user-rate", (req, res) => {
    Team.findByIdAndUpdate(
      req.body.id,
      {
        $inc: { "rateTeam.total": req.body.rate },
        $push: { "rateTeam.voters": req.body.email }
      },
      { new: true }
    )
      .then(team => {
        return team.updateRate(
          team.rateTeam.total,
          team.rateTeam.voters.length
        );
      })
      .then(team => {
        res.send(team);
