...
class DateAndTime extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.dateAndTime !== "") {
      if (pickType === "Daily") {
        this.setState(
          { startTime, endTime, pickType, isValid: true },
          () => {}
        );
      } else if (pickType === "Weekly") {
        const { dayOfTheWeekPicker, pickType } = this.props.dateAndTime;
        let tempStateDayCopyWeek = JSON.parse(
          JSON.stringify(this.state.dayOfTheWeekPicker)
        );
        ...
        this.setState(
          {
            dayOfTheWeekPicker: tempStateDayCopyWeek,
            isValid: true,
  ...
  checkValidFrom = () => {
    ...
    if (pickType === "Daily") {
      ...
      if (
        _.reject(_.map(dayOfTheWeekPicker, _.partialRight(_.pick, "check")), {
          check: false
        }).length > 0 &&
        startTime !== endTime
      ) {
  ...
  onClickCheckBoxHandler = event => {
    ...
    let dayOfTheWeekPicker = JSON.parse(
      JSON.stringify(this.state.dayOfTheWeekPicker)
    );
    for (let i = 0; i < dayOfTheWeekPicker.length; i++) {
  ...
  handleDayClick = (day, { selected }) => {
    ...
    //  console.log("selected",moment(moment(new Date()).format('MM/DD/YYYY')).isSameOrBefore(moment(day).format('MM/DD/YYYY')));
    if (selected) {
      ...
      if (
        moment(moment(new Date()).format("MM/DD/YYYY")).isSameOrBefore(
  ...
  onClickRadioHandler = value => {
    ...
    this.setState(
      { timePicker: timePicker, pickType: value, isValid: false },
      () => {
        this.checkValidFrom();
      }
    );
          ...
          <div className="main-date-time">
            <div className="main-controller-date-time-left">
              <div className="time-picker-general-wrapper">
                <div className="mobile-wrapper">
                  <div className="start-time-div">
                    ...
                    <TimePicker
                      ...
                      name="startTime"
                      showSecond={false}
                      onChange={e => this.onChangeTimeHandler(e, "start")}
                      allowEmpty={true}
                      className="start-time-date-time"
                      label="start"
                    />
                  </div>
                ...
                <div className="radio-btn-date-time-wrapper">{arrayRadio}</div>
              </div>
              <div className="main-controller-date-time-left-right">
                {pickType === "Weekly" && (
                  <div className="weekly-choice-date-time">
                    {arrayDayPicker}
                  </div>
                )}
                {pickType === "Monthly" && (
                  <div className="monthly-choice-date-time">
                    <DayPicker
            ...
            <div className="msg-date-time-contral-wrapper">
              <ContralTeamCreate
                class={this.state.pickType==="Daily" &&this.state.width <=800 ?"is-date-daily-date-and-time":"contral-team-date-time"}
...
const mapStateDispatch = dispatch => {
  return {
    saveDataAndTime: dateAndTime =>
      dispatch(actionType.saveDataAndTime(dateAndTime))
  };
};
