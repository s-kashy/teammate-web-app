...
class GeneralInfo extends Component {
  ...
  checkValidForm = () => {
    ...
    if (
      userBasic.isBasicInfoValid &&
      !aboutTheTeam.error &&
      imageUrl.valid &&
      typeOfSport !== arraySportType[0].toString()
    ) {
  ...
  checkBasicInfoValid = key => {
    ...
    if (key === "numberOfTeam") {
      if (
        userBasic[key].value === "" ||
        userBasic[key].value === undefined ||
        parseInt(userBasic[key].value) <= 0
      ) {
    ...
    return (
      <Aux>
        {!this.state.isLoading ? (
          <div className="main-general-info">
            <div className="from-general-info">
              <div className="option-menu-general-info">
                <OptionMenu
                  array={arraySportType}
                  selected={typeOfSport}
                  value={this.state.typeOfSport}
                  click={this.clickOptionChangeHandler}
                />
                <div className="upload-div-general-info">
                  <Input
                    ...
                    title={
                      imageUrl.fileName === "" ? "Team logo" : imageUrl.fileName
                    }
                  />
                  <span className="icon-upload-image-general-info">
                    <i
                      style={{
                        fontSize: "30px",
                        fontWeight: "1000",
              ...
              <div className="input-wrapper-general-info">
                ...
                <Input
                  type="number"
                  placeholder="Max of Team members"
                  name="numberOfTeam"
                ...
                <div>
                  <textarea
                    className="textarea-general-info"
                    onChange={event => this.onChangeTextAreaHandler(event)}
                    value={this.state.aboutTheTeam.value}
                    placeholder={
                      aboutTheTeam.error && aboutTheTeam.touch
                        ? "Don't forget*"
...
const mapStateDispatch = dispatch => {
  return {
    saveGeneralInfo: generalInfo =>
      dispatch(actionType.saveGeneralInfo(generalInfo))
  };
};
